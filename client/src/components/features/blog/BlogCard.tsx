...
import { Heart, MessageCircle,ThumbsUp,Bookmark,MoreHorizontal } from "lucide-react";
import { cn } from "@/lib/utils";
import type { LayoutType } from "../../common/subComps/layout-toggle";
import type { Blog } from "@/types/api"; // 1. Import the actual Blog type from your API types

interface BlogCardProps {
  blog: Blog; // 2. Expect the real Blog type
  layout?: LayoutType;
}
...
const BlogCard: React.FC<BlogCardProps> = ({ blog, layout = "square" }) => {
  ...
  // 3. Provide default values for interaction counts, as they may not be on the main blog object yet.
  ...
  return (
    <Link
      to={`/blog/${blog.slug}`}
      className={cn(
        "group block w-full overflow-hidden rounded-2xl bg-card border border-border shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1",
    ...
    >
      ...
      <div
        className={cn(
          ...
          isLandscape
            ? "w-full sm:w-2/5 md:w-1/3 flex-shrink-0"
            : "w-full aspect-video"
        )}
      >
        <img
          src={blog.thumbnail} // 4. Use `thumbnail` from the API model
          alt={blog.title}
      ...
      <div className="flex flex-col justify-between p-4 sm:p-5 flex-grow">
        <div>
          <span className="text-xs font-semibold uppercase text-primary tracking-wider">
            ...
            {/* 5. Use the categories array */}
          </span>
          <h3
            className={cn(
              "font-bold text-foreground mt-2 line-clamp-2",
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-border/60">
          <div className="flex items-center">
            {/* 6. Access the nested `owner` object for author details */}
            <img
